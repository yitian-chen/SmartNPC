...
- **combat-detach 战斗让位（2026-09-22）**：攻击事件携带 `detach:true` = **UE 战斗 AI 接管身体，agent 让位**——与 force 反应语义相反（实测 bug：MCP 1 秒内重规划下发逃跑动作，UE 战斗 AI 从未拿到控制权）。入口 `handleCombatDetach`（handleForceEvent 首分支）：掐在途 LLM + **不发 stop_action**（stop 的 UE 语义是"中断行为树"，迟到的 stop 会把 UE 刚启动的战斗接管连同旧动作一起杀掉——2026-09-22 呆站根因假设；停旧动作由 UE 接管自含，入口只记住在途 action_id 供归还补 stop）+ `ClearForReplan`（清队列/slot/在途 stash，**事件队列保留**）+ 取消 timer + `clearReaction` + 清 slotSwitchPending + 置 `combatYield`（agentContext coordMu，镜像 reactionActive）；**不重规划不下发不 hint**，重复攻击幂等（仅刷新 TTL 起点与 lastEvent）。让位期间抑制面：worker 守卫（processSlotSwitch **之前**——它会清 slot+queue）挡 slot 切换/pop/refill（`checkCombatYieldExpiry` 在守卫内查 TTL）、detectDayRollover 跳过（电平检测自愈）、`route()`/`routerInterrupt()` 顶部守卫（掐断合成事件第二发射点与在途 jev 竞态，事件留队列）、chat_invite 礼貌拒绝（sendInviteRsp false）、guardedExecutor.SendAction 拒绝、/debug/schedule 409（/debug/action 放行带警告）。**归还**：让位中的 `combat_exit` 在 handleWorldEvent 确定性接收（不走路由——让位期间无在途动作无反应上下文，jev 必误判）；TTL 兜底 30 游戏分钟（`combatYieldTTLGameSec`，无感知时 inert + 首感知锁存，防冷启动误触发）。归还路径 `reclaimFromCombatYield`：清让位 + 对记下的在途动作补一次精确 stop（UE 已停则 STOP_ID_MISMATCH 无害，防战后新动作被 busy 拒）+ Echo 攻击事件回声（"再见一次"语义，攻击→脱战 FIFO 对进【发生的事件】）+ `SetReplanHint(【战斗结束】…)`（tacticalHintLine 渲染战后恢复指引）+ `forceInterruptReplan`（slot 已清无妨——`selectCurrentGoal` 按游戏时间重推）。让位状态**跨 UE 断线重连存续**（stop() 不清：UE 重连不重发 detach，清了就在每次 WS 抖动后抢回身体；TTL 有界兜底）。协议容错：UE 首版发 `event_type:"attacked"` 短名且缺 category——`pkg/prompt` 别名表 + `EffectiveCategory`/`EffectiveEventType` 统一归一（label+description 双推断），`IsCombatStartEvent`/`IsCombatExitEvent` 匹配 category 空或 player_interaction + 别名，威胁情境登记不再漏
...
7. **反应层冷启动超时**：Ollama 模型卸载后首 call >8s 超时（反应层当前默认禁用，影响有限）
