...
| detach | bool | ❌ | **战斗接管标记**（combat-detach）。true = UE 战斗 AI 接管该 NPC，Agent 让位（不重规划、不下发任何动作；在途动作由 UE 接管在宽限期内自行停止，超时未停则 Agent 代为 stop），直到 `combat_exit`（或 Agent 侧 30 游戏分钟 TTL 兜底）归还控制权。仅 `player_attacked`/`player_targeted` 携带，省略 = false |
...
5. **detach 例外（combat-detach）**：`force=true` 且 `detach=true` 的战斗事件，Agent 侧承诺的是**让位**而非反应——不重规划、不下发任何动作，把身体完整交给 UE 战斗 AI；slot 切换挂起、路由静默、对话邀请礼貌拒绝。**在途动作的停止分两段**：让位后的宽限期内（2 秒 wall-clock）Agent 不发 stop_action（stop 的 UE 侧语义是"中断行为树"，迟到的 stop 会把刚启动的接管连同旧动作一起杀掉；UE 接管应自行停掉旧动作并回 `action_completed{interrupted}`，Agent 收到即免 stop）；宽限期过后旧动作仍未结束（接管未触发）则 Agent 代为 stop 一次，使脱管在物理上成立（机器人停止 agent 驱动的行为）。控制权由 `combat_exit` 归还（确定性接收，不经路由裁决），若旧动作仍未停则再补一次精确 stop（UE 已停则回 STOP_ID_MISMATCH，无害），随即带战后上下文（物理状态变化 + 攻击/脱战事件对）重规划回日程；**归还链的第二跳触发当日剩余时段的战略层修订**（2026-09-23）——战斗导致的属性显著变化（如关节磨损暴涨）正是"早晨计划对当下无知"的典型案例，Agent 以让位入口为基线计算战斗期间属性变化（磨损/疲劳/电量 before→after），随【战斗结束】hint 注入战后首次分解、并作为修订原因驱动剩余时段重排（如优先插入维修/充电时段）；共享既有节流（每游戏日上限 5 次 + 相邻间隔 ≥2 游戏小时），让位中不规划。**不可否决**对 detach 同样成立：Agent 侧没有任何逻辑能拒绝让位。
