...
// 战略层 prompt 与第一次战术层 prompt（system + user 全文）覆盖写入
...
//   - 每次进程首次落盘时覆盖清空旧内容（O_TRUNC），进程内后续 layer
//     追加写入（O_APPEND），文档只反映最新一次仿真
...
func dumpPromptDoc(agentID, layer, system, user string, logger *slog.Logger) {
	...
	// 进程内首次落盘：覆盖清空旧内容（O_TRUNC）；后续 layer 追加（O_APPEND）。
	...
	f, err := os.OpenFile(promptDocPath, flags, 0o644)
	if err != nil {
