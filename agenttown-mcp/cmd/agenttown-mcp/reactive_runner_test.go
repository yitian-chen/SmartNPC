...
func TestUpgradeIfPhysicalAlert_ReasonContainsOrigReaction(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			input:        ReactiveInput{Fatigue: 82, Energy: 70, PhysicalAvailable: true},
		...
		},
		{
			...
			input:        ReactiveInput{Fatigue: 82, Energy: 50, PhysicalAvailable: true},
...
func TestUpgradeIfPhysicalAlert(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			input:    ReactiveInput{Fatigue: 30, Energy: 25, PhysicalAvailable: true},
			dec:      ReactiveDecision{Reaction: ReactionContinue, Reason: "继续行动"},
			wantKind: ReactionReplan,
			wantSub:  "物理状态告警自动升级",
		},
		{
			...
			input:    ReactiveInput{Fatigue: 30, Energy: 70, JointWear: 75, PhysicalAvailable: true},
			dec:      ReactiveDecision{Reaction: ReactionObserve, Reason: "状态尚可"},
			wantKind: ReactionReplan,
			wantSub:  "关节磨损",
		},
		{
			...
			input:    ReactiveInput{Fatigue: 30, Energy: 70, JointWear: 70, PhysicalAvailable: true},
			dec:      ReactiveDecision{Reaction: ReactionObserve, Reason: "ok"},
			wantKind: ReactionObserve,
		},
		{
			name:     "no alert preserves observe",
			input:    ReactiveInput{Fatigue: 50, Energy: 70, PhysicalAvailable: true},
			dec:      ReactiveDecision{Reaction: ReactionObserve, Reason: "状态尚可"},
			wantKind: ReactionObserve,
		},
		{
		...
		},
		{
			...
			input:    ReactiveInput{Fatigue: 80, Energy: 70, PhysicalAvailable: true},
			dec:      ReactiveDecision{Reaction: ReactionObserve, Reason: "ok"},
			wantKind: ReactionObserve,
		},
		{
			name:     "physical unavailable skips upgrade even if values alert",
			input:    ReactiveInput{Fatigue: 82, Energy: 25, PhysicalAvailable: false},
			dec:      ReactiveDecision{Reaction: ReactionObserve, Reason: "状态尚可"},
			wantKind: ReactionObserve,
		},
	}
