...
请基于你的角色身份和性格，规划今天一天的活动安排。一天从 07:00 到次日 06:00，你从 07:00 开始活动，夜间活动可持续到次日清晨。
...
8. 末段若跨午夜（如 "22:00-06:00"），结束时间表示次日该时刻，调度器会自动识别跨午夜时段
9. goal 的主要活动应能用【可用能力】中列出的复合动作实现（如装配→work_at_workbench、充电→charge_at_station）。若活动无对应专用复合动作（如"整理仪容""冥想""拉伸"等），可由 generic_act 通用动作兜底实现；但不得编造【可用能力】未列出且无合理实现方式的活动
