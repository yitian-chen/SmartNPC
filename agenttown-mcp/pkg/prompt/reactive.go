...
// than at 11:00. energy kept at early-warning levels so 1-2 hour
...
func BuildReactive(in ReactiveInput) string {
	...
	if in.PhysicalAvailable {
		...
		physicalRuleLine = "【物理告警补充规则】当【当前状态】中电量为\"低电量\"、疲劳度为\"非常疲劳\"或关节磨损度为\"严重磨损\"时，原则上需要输出 replan 让 NPC 优先处理物理需求、不可输出 continue/observe\n"
	}
...
// alert (fatigue>80 / energy<40 / joint_wear>70) but the LLM still
