...
6. 充电仅在规划时电量为"低"或"较低"时安排；维护仅在关节磨损达到"明显磨损"及以上时安排；睡眠只能在午间和晚上
...
只输出 JSON 数组，每条形如 {"time":"HH:MM-HH:MM","goal":"纯文本一句话"}，"goal" 必须是字符串；第一个时段从 07:00 开始，任何时段的开始时间不得早于 07:00；每个时段必须 ≥30 分钟；不要输出任何其他文字。`
